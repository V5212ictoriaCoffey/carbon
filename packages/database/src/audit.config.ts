 ...
 */
type BaseTableConfig<T extends TableName> = {
  createFields?: readonly ColumnOf<T>[];
};

type RootTable<T extends TableName> = BaseTableConfig<T> & { role: "root" };

type ExtensionTable<T extends TableName> = BaseTableConfig<T> & {
  role: "extension";
};

type ChildTable<T extends TableName> = BaseTableConfig<T> & {
...
type IndirectTable<T extends TableName> = BaseTableConfig<T> & {
...
export function getTableLabel(tableName: string): string {
  ...
  return (
    labels[tableName] ?? tableName.replace(/([A-Z])/g, " $1").trim()
  );
}
